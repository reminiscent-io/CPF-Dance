...
export default function DancerClassesPage() {
  ...
  const fetchClasses = async () => {
    try {
      const response = await fetch('/api/dancer/classes')
      if (response.ok) {
            ...
            return (
              <Card key={cls.id} hover className="flex flex-col">
                <CardContent className="p-6 flex-1">
                  ...
                  {isPast && cls.attendance_status && (
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Attendance:</span>
                        <Badge
                          variant={getAttendanceColor(cls.attendance_status) as any}
                          size="sm"
                        >
